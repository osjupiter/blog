...
- The generator reads each article's `<title>` and first `<p class="subtitle">`,
  and takes published/updated dates from the article's JSON-LD
  `datePublished`/`dateModified` (edit those to adjust publish dates;
  git history is only the fallback when they are missing).
